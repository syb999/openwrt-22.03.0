...
$(eval $(call SetupHostCommand,gcc, \
	Please install the GNU C Compiler (gcc) 5 or later, \
	$(CC) -dumpversion | grep -E '^([5-9]\.?|1[0-9]\.?)', \
	gcc -dumpversion | grep -E '^([5-9]\.?|1[0-9]\.?)', \
	gcc --version | grep -E 'Apple.(LLVM|clang)' ))

$(eval $(call TestHostCommand,working-gcc, \
	Please reinstall the GNU C Compiler (5 or later) - \
	it appears to be broken, \
	echo 'int main(int argc, char **argv) { return 0; }' | \
		gcc -x c -o $(TMP_DIR)/a.out -))

$(eval $(call SetupHostCommand,g++, \
	Please install the GNU C++ Compiler (g++) 5 or later, \
	$(CXX) -dumpversion | grep -E '^([5-9]\.?|1[0-9]\.?)', \
	g++ -dumpversion | grep -E '^([5-9]\.?|1[0-9]\.?)', \
